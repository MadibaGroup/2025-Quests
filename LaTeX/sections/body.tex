...
\paragraph{Reproducibility.} For our paper, we study a quest system built and maintained by a company offering a new blockchain. The quests themselves were co-designed with various services (DApps) running on the company's blockchain. The choice to use proprietary data has pros and cons. 
...
\paragraph{Representativeness.} Our dataset originates from a single platform, which means we cannot assert broad generalizability. However, the platform's quests and bot-control measures appear standard and comparable to other blockchain quest systems we examined. Our data covers both pre- and post-airdrop periods, enabling us to observe user behaviour changes around significant events. It's important to note that our findings may become less representative over time due to the increasing ease with which non-experts can create bots, particularly as AI-driven tools and autonomous agents such as ElizaOS~\cite{} become more prevalent.
...
\begin{itemize}
\item Heuristic 1: If a quest alternates between unprofitable and profitable, bots will be sensitive to the exact moment it becomes marginally profitable and act.\item Heuristic 2: If a quest requires a threshold (minimal amount of tokens), bots will be sensitive to ensuring the address meets but does not exceed the threshold amount. 
...
% other programs have similar quests, main difference is it’s on another chain, we don’t a reason why we wouldn’t generalize.
%if quest is different, in our model we did abstract the type of quest to like the factors that could impact them like difficulty,.. [we don’t if it applies, conjecture]
%If results were very different, over time it would change, advances in ai. Our results from 2024 don’t necessarily translate to 2028 for bots, but for humans they probably do. 
%Where do we add this? JWC

%Future work
% ground truth about bots and humans
% The authors should put more effort into the identification of bots. I agree that this is a challenging task, but many of the findings are severely impaired by the lack of this data. In addition to the «number of distinct completions», variables like task difficulty can be completely off, if for example the external dependency is in fact just a coordination between 2 bot instances of the same entity.
...
%%Future studies could monitor 

%While similar data could be approximated by monitoring the user interface and manually recording point allocations, this method risks inaccuracies since some qualifying transactions may not originate from the loyalty program's interface.
